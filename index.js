...
const $ = {
  bodyClassAdd: (c) => $.el("div.left-container").classList.add(c),
  bodyClassRemove: (c) => $.el("div.left-container").classList.remove(c),
...
class HistoryInfluencer extends Influencer {
  ...
  _getHistory() {
    this._history =
      this._history || JSON.parse(localStorage.getItem(this._storeName)) || [];
...
class Suggester {
  ...
  _rehighlight() {
    ...
    this._highlight($.el(`[data-suggestion="${this._highlightedSuggestion}"]`));
  }
